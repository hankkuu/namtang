...
//		/*
//		 * Optional<AccountModel> accountModelOptional =
//		 * repository.findByUsername(username); if(accountModelOptional.isPresent()) {
//		 * AccountModel m = accountModelOptional.get(); String[] roles =
//		 * m.getRoles().stream().map(RoleModel::getName).map(x ->
//		 * x.name()).toArray(String[]::new); // test 로 진행하느라 password encorder를 여기서 사용.
//		 * NoOpPasswordEncoder으로 변경 // return
//		 * User.builder().username(m.getUsername()).password(passwordEncoder.encode(m.
//		 * getPassword())).roles(roles).build(); return
//		 * User.builder().username(m.getUsername()).password(m.getPassword()).roles(
